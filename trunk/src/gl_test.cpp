...
void renderHelp()
{
  char* help[]={"ESC: quitter",
                ...
                "n: passer à la camera suivante"};
  unsigned int nb_lines=6;
  glColor3f(1,1,1);
...
void Idle(void)
{
  //MAJ de l'orienation du teapot
  object->getTransform()->setOrientation(xa,ya,za);
  xa+=dt;
  ya+=dt;
  za+=dt;
...
void init()
{
  ...
  shape=new Shape();
  object->setShape(shape);
  ShapePart* pot=new Teapot();
  pot->getTransform()->setScale(10,10,10);
  shape->addPart(pot);
  scene.addEntity(object);
}
